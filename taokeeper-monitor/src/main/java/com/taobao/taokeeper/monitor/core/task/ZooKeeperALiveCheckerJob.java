    ...
    public void run() {
        ...
        while (true) {
            ...
            if (!GlobalInstance.need_node_alive_check) {
                LOG.info("No need to node_alive_check, need_node_alive_check="
                        + GlobalInstance.need_node_alive_check);
                try {
                    Thread.sleep(1000 * 60 * MINS_RATE_OF_COLLECT_HOST_PERFORMANCE);
                } catch (InterruptedException e) {
                    LOG.error("Sleep failure: " + e.getMessage());
                }
                continue;
            }
            try {
                ...
                try {
                    ...
                    if (null == zooKeeperClusterSet || zooKeeperClusterSet.isEmpty()) {
                        ...
                        for (final ZooKeeperCluster zookeeperCluster : zooKeeperClusterSet) { // 对每个cluster处理
                            ...
                            // 如果已经有人在检测了
                            if (!GlobalInstance.addToAllCheckingCluster(zookeeperCluster.getClusterId()
                                    + EMPTY_STRING)) {
                                // 那么不检测了。
                                continue;
                            }
                            ...
                            if (null != zookeeperCluster && null != zookeeperCluster.getServerList()) {
                                ...
                                final AlarmSettings alarmSettings = alarmSettingsDAO
                                        .getAlarmSettingsByCulsterId(zookeeperCluster.getClusterId());
                                if (null == alarmSettings) {
                                    // 一定要进行释放。
                                    GlobalInstance.removeFromAllCheckingCluster(zookeeperCluster
                                            .getClusterId() + EMPTY_STRING);
                                    continue;
                                }
                                ThreadPoolManager
                                        .addJobToZooKeeperNodeAliveCheckExecutor(new ZKServerAliveCheck(
                                                zookeeperCluster, alarmSettings));
                            }// if cluster not null
                        }// for each cluster
                    }// for each server
